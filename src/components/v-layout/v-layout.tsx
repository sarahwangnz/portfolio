                ...
                <Nav className="layout-header-end me-end">
                    {
                        group2.length > 0 ?
                            group2.map((routeItem, idx) => {
                                return routeItem.type === 'link' ? <Nav.Link id={"nav-end-" + idx} key={"menu_item_" + idx} href={v_link(routeItem.route)}>{routeItem.name}</Nav.Link> :
                                    <span className="v-divider">{routeItem.name}</span>
                            }
                            ) : ""
                    }
                </Nav>
